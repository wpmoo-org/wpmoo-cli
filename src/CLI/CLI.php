...
class CLI extends Base {
    ...
    public static function run($argv) { (new self())->handle($argv); }

    public function handle(array $argv) {
        $command = isset($argv[1]) ? (string) $argv[1] : 'help';
        if ('help' === $command || ! isset($this->commands[$command])) { $this->renderHelp(); return 0; }
    ...
    protected function renderHelp() {
        ...
        $width = 0; foreach (array_keys($definitions) as $cmd) { $width = max($width, strlen($cmd)); }
        foreach ($definitions as $cmd => $desc) { $padding = str_repeat(' ', $width - strlen($cmd) + 2); Console::line('  ' . $cmd . $padding . $desc); }
        Console::line();
    }
    ...
    protected function renderWelcome() {
        ...
        Console::line(); foreach ($this->logoLines() as $line) { Console::banner($line); }
        $version = $summary['version'] ? $summary['version'] : 'dev'; Console::comment('WPMoo Version ' . $version); Console::line();
        $wp_cli = $summary['wp_cli_version'] ? $summary['wp_cli_version'] : 'not detected'; Console::comment('→ WP-CLI version: ' . $wp_cli);
