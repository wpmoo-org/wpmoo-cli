...
class Base {
    ...
    protected static function framework_base_path() { return defined('WPMOO_PATH') ? rtrim(WPMOO_PATH, '/\\') . DIRECTORY_SEPARATOR : self::base_path(); }
    protected static function base_path() { $app = App::instance(); return rtrim($app->path(''), '/\\') . DIRECTORY_SEPARATOR; }
    ...
    protected function detect_project_metadata($path) {
        ...
        foreach (glob(rtrim($path,'/\\') . DIRECTORY_SEPARATOR . '*.php') as $file) {
            ...
            if (preg_match('/^[ \t\/*#@]*Plugin Name:\s*(.*)$/mi', $c)) { $metadata['main']=$file; $metadata['slug']=basename($file,'.php'); break; }
        }
        $composer = rtrim($path,'/\\') . DIRECTORY_SEPARATOR . 'composer.json';
        if (file_exists($composer)) { $raw = @file_get_contents($composer); if ($raw!==false) { $d = json_decode($raw,true); if (is_array($d)) { $metadata['name']=$d['name'] ?? $metadata['name']; $metadata['version']=$d['version'] ?? $metadata['version']; } } }
        if (empty($metadata['slug'])) { $metadata['slug'] = basename(rtrim($path,'/\\')); }
        return $metadata;
    }

    protected function detect_primary_namespace($base_path) {
        $composer = rtrim($base_path,'/\\') . DIRECTORY_SEPARATOR . 'composer.json'; if (!file_exists($composer)) return null;
        $raw = @file_get_contents($composer); if ($raw===false) return null; $d = json_decode($raw,true); if (!is_array($d)) return null;
        $psr4 = $d['autoload']['psr-4'] ?? ($d['autoload-dev']['psr-4'] ?? array()); if (!is_array($psr4) || empty($psr4)) return null;
        $keys = array_keys($psr4); return rtrim((string)$keys[0],'\\');
    }

    protected function detect_current_version($base_path) {
        $composer = rtrim($base_path,'/\\') . DIRECTORY_SEPARATOR . 'composer.json';
        if (file_exists($composer)) { $raw = @file_get_contents($composer); if ($raw!==false) { $d=json_decode($raw,true); if (is_array($d) && isset($d['version'])) return (string)$d['version']; } }
        return null;
    }
}
