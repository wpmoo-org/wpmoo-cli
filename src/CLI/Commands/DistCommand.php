...
class DistCommand extends Base implements CommandInterface {
    public function handle(array $args = array()) {
        ...
        if (!$source_root || !is_dir($source_root)) { Console::error('Unable to resolve source directory for distribution.'); return 1; }
        ...
        if ($options['label']) { $slug = $this->sanitize_slug($options['label']); }
        elseif ($is_framework) { $slug = $this->plugin_slug(); }
        elseif (!empty($metadata['slug'])) { $slug = $metadata['slug']; }
        else { $slug = $this->sanitize_slug(basename($source_root)); }
        ...
        if (!$dist_root) { Console::error('Failed to resolve distribution output directory.'); return 1; }
        if (!$this->ensure_directory(rtrim($dist_root, '/\\') . DIRECTORY_SEPARATOR)) { Console::error('Unable to create distribution output directory.'); return 1; }
        $temp_dir = $this->create_temp_directory($slug . '-dist-'); if (!$temp_dir) { Console::error('Unable to create temporary directory for distribution build.'); return 1; }
        $target_root = $temp_dir . DIRECTORY_SEPARATOR . $label; if (!@mkdir($target_root, 0755, true)) { Console::error('Unable to prepare working directory for distribution.'); $this->delete_directory($temp_dir); return 1; }
        Console::line(); Console::comment('Preparing distribution: ' . $label);
        if ($is_framework) {
            foreach ($this->default_dist_includes($source_root) as $entry) {
                $source = $source_root . DIRECTORY_SEPARATOR . $entry; $target = $target_root . DIRECTORY_SEPARATOR . $entry;
                if ($this->copy_within_dist($source, $target) && 'vendor' === $entry) { $this->prune_vendor_tree($target); }
                if (!file_exists($target)) { Console::warning('Failed to include ' . $entry . ' in distribution.'); }
            }
            ...
            } else { Console::comment('→ Composer binary not found; reusing existing vendor directory.'); }
            ...
            if (!$this->copy_tree($source_root, $target_root, $exclusions)) { Console::error('Failed to copy project files into working directory.'); $this->delete_directory($temp_dir); return 1; }
            $this->post_process_deploy($target_root, array());
            $this->prune_vendor_tree($target_root . DIRECTORY_SEPARATOR . 'vendor');
        }
        $archive_path = rtrim($dist_root, '/\\') . DIRECTORY_SEPARATOR . $label . '.zip';
        if (!$this->create_zip_archive($target_root, $archive_path)) { Console::error('Failed to create distribution archive.'); $this->delete_directory($temp_dir); return 1; }
        Console::info('Distribution archive created: ' . $this->relative_path($archive_path));
        $this->do_action_safe('wpmoo_cli_dist_completed', array('label'=>$label,'version'=>$version,'archive'=>$archive_path,'path'=>$target_root,'source'=>$source_root,'options'=>$options));
        if (!$options['keep']) { $this->delete_directory($temp_dir); } else { Console::comment('Working directory preserved at ' . $this->relative_path($temp_dir)); }
