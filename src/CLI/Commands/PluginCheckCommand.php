...
class PluginCheckCommand extends Base implements CommandInterface {
    public function handle( array $args = array() ) {
        ...
        list( $code, $out ) = $this->execute_command( 'wp', $args, null );
    ...
    private function parseOptions( array $args ) {
        ...
        foreach ( $args as $arg ) {
            if ( 0 === strpos( $arg, '--path=' ) ) { $opts['path'] = substr( $arg, 7 ); }
            if ( 0 === strpos( $arg, '--slug=' ) ) { $opts['slug'] = substr( $arg, 7 ); }
            if ( '--no-strict' === $arg ) { $opts['strict'] = false; }
            if ( 0 === strpos( $arg, '--ignore-codes=' ) ) { $opts['ignore'] = substr( $arg, 15 ); }
        }
        return $opts;
    }
    ...
    private function render_table( array $rows ) {
        ...
        $pad = function( $s, $w ) { $l = strlen( $s ); return $s . str_repeat( ' ', max( 0, $w - $l ) ); };
        // Header
        ...
        // Rows
        foreach ( $items as $it ) {
            $type = $it[0]; $code = $it[1]; $msg = $it[2];
            $line = '  ' . $pad( $type, $w1 ) . '  ' . $pad( $code, $w2 ) . '  ' . $msg;
            if ( 'ERROR' === $type ) { Console::error( $line ); }
            elseif ( 'WARNING' === $type ) { Console::warning( $line ); }
            else { Console::line( $line ); }
        }
        // Summary
        $summary = array();
        if ( $errors ) { $summary[] = 'Errors: ' . $errors; }
        if ( $warnings ) { $summary[] = 'Warnings: ' . $warnings; }
        Console::line();
        if ( $summary ) { Console::comment( implode( '  ', $summary ) ); }
        return $errors > 0; // failed if any errors
    }
}
